...
def verify_elf(name: str, data: bytes) -> None:
    ...
    for index in range(count):
        kind, _, offset, address, _, _, _, alignment = struct.unpack_from('<IIQQQQQQ', data, table + index * stride)
...
def verify(apk: Path, *, debug: bool, signed: bool) -> None:
    ...
    signature = subprocess.run([str(BUILD_TOOLS / 'apksigner'), 'verify', str(apk)],
                               capture_output=True)
    if (signature.returncode == 0) != signed:
        raise RuntimeError(f'{apk}: expected signed={signed}, signature exit={signature.returncode}')
    print(f'PASS {apk.name}: universal ARM64+x86-64, debug={debug}, signed={signed}, ZIP/16 KiB ELF aligned')
