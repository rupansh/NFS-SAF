...
try:
    result = subprocess.run(
        [args.binary, "127.0.0.1", "/", "42"],
        env=dict(
            os.environ, NFS_TEST_PORT=str(server.server_address[1]), NFS_TEST_TIMEOUT_MS="3000"
        ),
        capture_output=True,
        text=True,
        timeout=12,
            ...
            while data := self.request.recv(1024 * 1024):
                ...
                while len(pending) >= 4:
                    ...
                    if len(pending) < 4 + length:
                        break
                    record.extend(pending[4 : 4 + length])
...
try:
    result = subprocess.run(
        [args.binary, "127.0.0.1", args.export, "42"],
        env=dict(
            os.environ,
            NFS_TEST_PORT=str(server.server_address[1]),
            NFS_TEST_UID="4000000000",
            NFS_TEST_GID="3000000000",
            NFS_TEST_GROUPS="yes",
        ),
        capture_output=True,
        text=True,
    ...
    print(
        f"PASS AUTH_SYS uid/gid/supplementary-gids: {len(credentials)} RPC calls; {result.stdout.strip()}"
    )
finally:
    server.shutdown()
    server.server_close()
