...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix='nfssaf-sign-test-') as temporary:
        ...
        key_args = ['-keystore', str(key), '-storepass:env', 'ANDROID_KEYSTORE_PASSWORD',
                    '-alias', passwords['ANDROID_KEY_ALIAS']]
        subprocess.run([
            'keytool', '-genkeypair', '-storetype', 'JKS', *key_args,
            '-keypass:env', 'ANDROID_KEY_PASSWORD', '-keyalg', 'RSA', '-keysize', '2048',
            '-validity', '2', '-dname', 'CN=Disposable NFS SAF signing test',
        ], env=env | passwords, check=True, capture_output=True)
        values = passwords | {'ANDROID_KEYSTORE_BASE64': base64.b64encode(key.read_bytes()).decode('ascii')}
        ...
        certificate = subprocess.run(['keytool', '-exportcert', *key_args], env=env | passwords,
                                     check=True, capture_output=True).stdout
        expected = hashlib.sha256(certificate).hexdigest()
        apksigner = Path(env['ANDROID_HOME']) / 'build-tools/36.1.0/apksigner'
        verified = subprocess.run([str(apksigner), 'verify', '--print-certs', str(output)],
        ...
        assert hashlib.sha256(source.read_bytes()).digest() == original_digest, 'Unsigned input modified'
