...
def main() -> None:
    ...
    parser.add_argument('--check-config', action='store_true', help='Check required secrets before CI builds')
    ...
    with tempfile.TemporaryDirectory(prefix='nfssaf-sign-', dir=os.environ.get('RUNNER_TEMP')) as temp:
        temp_dir = Path(temp)
        ...
        subprocess.run([
            sys.executable, str(root / 'scripts/verify-apks.py'),
            '--release', str(signed), '--signed-release',
        ], check=True, capture_output=True)
...
if __name__ == '__main__':
    try:
        ...
        sys.exit('Release signing/verification failed. Check the keystore, alias and passwords; no release was published.')
