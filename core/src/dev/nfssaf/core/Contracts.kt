...
value class ShareId(val value: String) {
    ...
    companion object {
        fun random() = ShareId(java.util.UUID.randomUUID().toString())
    }
}
...
value class DocumentId(val value: String) {
    ...
    companion object {
        fun random() = DocumentId(java.util.UUID.randomUUID().toString())
    }
}
...
value class RemotePath(val value: String) {
    ...
    val parent
        get() = RemotePath(Paths.parent(value))
...
enum class Protocol(val wire: Int, val label: String) {
    V3(3, "NFS 3"),
    V4(4, "NFS 4.0"),
    V42(42, "NFS 4.2"),
}
...
sealed interface Node {
    ...
    data class Directory(
        ...
        override val share: ShareId,
        override val path: RemotePath,
...
internal constructor(
    ...
    private fun live(): State.Open =
        state as? State.Open ?: throw NfsException(Errno.EBADF, "File is closed")

    @Synchronized fun size(): Long = live().size

    @Synchronized
    internal fun readAt(offset: Long, size: Int, data: ByteArray): Int {
        live()
        validateRange(offset, size, data)
        if (size == 0) return 0
        return io.read(offset, size, data)
    }

    @Synchronized
    internal fun writeAt(offset: Long, size: Int, data: ByteArray): Int {
        val current = live()
        validateRange(offset, size, data)
        if (size == 0) return 0
        ...
        val validCount = receipt.count == size && receipt.endOffset >= size
        val validPosition = append || receipt.endOffset == position + size
        if (!validCount || !validPosition)
            throw NfsException(Errno.EIO, "Backend returned an invalid write receipt")
...
fun <A : Readable> FileHandle<A>.read(offset: Long, size: Int, data: ByteArray) =
    readAt(offset, size, data)

fun <A : Writable> FileHandle<A>.write(offset: Long, size: Int, data: ByteArray) =
    writeAt(offset, size, data)
...
sealed interface OpenedFile : AutoCloseable {
    ...
    companion object {
        fun own(io: FileIo, mode: OpenMode, size: Long): OpenedFile =
            when (mode) {
                OpenMode.READ -> ReadOnly(FileHandle(io, size))
                OpenMode.WRITE,
                OpenMode.APPEND -> WriteOnly(FileHandle(io, size, mode.append))
                OpenMode.READ_WRITE,
                OpenMode.REPLACE -> ReadWrite(FileHandle(io, size))
            }
    }
}
