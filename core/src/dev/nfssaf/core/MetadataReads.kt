...
class MetadataReads(
    ...
    fun <T> run(read: (MetadataReader) -> T): T = try { once(read) } catch (e: NfsException) {
        if (!e.isTransportFailure) throw e
        onRetry(e)
        once(read)
    }

    private fun <T> once(read: (MetadataReader) -> T): T = acquire().use { lease ->
        try { read(lease.value) } catch (e: NfsException) {
            if (e.isTransportFailure || e.errno == 116) lease.reusable = false
            throw e
        }
    }
}
...
val NfsException.isTransportFailure: Boolean
    get() = errno in setOf(5, 32, 100, 101, 102, 103, 104, 110, 112, 113)
