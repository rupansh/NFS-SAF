...
/** Complete snapshots only. The first caller loads on its own worker thread;
 * concurrent callers share that load without holding the cache monitor during IO.
 * Eviction/invalidation detaches a load so its completion cannot repopulate the cache.
 */
class SnapshotCache<K, V>(
    ...
    private val entries = object : LinkedHashMap<K, State<V>>(capacity, 0.75f, true) {
        override fun removeEldestEntry(eldest: MutableMap.MutableEntry<K, State<V>>) = size > capacity
    }

    fun get(key: K, load: () -> V): V {
        val pending = synchronized(entries) {
            ...
            State.Loading(FutureTask { State.Ready(load(), clock()) }).also { entries[key] = it }
        }
        // FutureTask runs at most once, even when several callers arrive together.
        pending.task.run()
        val result = try { pending.task.get() } catch (e: ExecutionException) {
