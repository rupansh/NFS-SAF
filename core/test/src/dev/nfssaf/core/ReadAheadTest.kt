...
class ReadAheadTest {
    ...
    @Test
    fun sequentialReadsCollapseRpcsAndPreserveBytes() {
        val s = Source()
        val r = ReadAhead(s, { 0 }, windowBytes = 4096)
        repeat(20) {
            val data = ByteArray(128)
            assertEquals(128, r.read(it * 128L, 128, data))
            assertArrayEquals(s.bytes.copyOfRange(it * 128, it * 128 + 128), data)
        }
        assertEquals(listOf(128, 4096), s.requests)
    }

    @Test
    fun randomAccessDoesNotSpeculativelyFetch() {
        val s = Source()
        val r = ReadAhead(s, { 0 }, windowBytes = 4096)
    ...
    @Test
    fun writesAndExpiryInvalidateCachedBytes() {
        var time = 0L
        var revision = 0L
        val s = Source()
        val r = ReadAhead(s, { revision }, { time }, 4096, 250)
        ...
        revision++
        r.read(200, 100, data)
        assertEquals(99, data[0].toInt())
        s.bytes[300] = 88
        time = 251
        r.read(300, 100, data)
        assertEquals(88, data[0].toInt())
        assertEquals(4, s.calls)
    }

    @Test
    fun eofAndBufferBoundaryRemainCorrect() {
        val s = Source()
        val r = ReadAhead(s, { 0 }, windowBytes = 4096)
        r.read(19800, 100, ByteArray(100))
        assertEquals(100, r.read(19900, 100, ByteArray(100)))
