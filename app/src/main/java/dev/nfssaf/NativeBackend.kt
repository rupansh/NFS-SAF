...
import dev.nfssaf.core.FileIo
import dev.nfssaf.core.LeasePool
import dev.nfssaf.core.MetadataReader
import dev.nfssaf.core.MetadataReads
...
import dev.nfssaf.core.WriteReceipt
import dev.nfssaf.core.isTransportFailure
import dev.nfssaf.core.read
import dev.nfssaf.core.write
import java.util.concurrent.ConcurrentHashMap

// JNI signatures mirror the C ABI; typed capabilities are enforced by NativeSession.
@Suppress("TooManyFunctions", "LongParameterList")
internal object NativeBridge {
    init {
        System.loadLibrary("nfssaf")
    }
...
internal class NativeSession(private val share: Share) : AutoCloseable, MetadataReader {
        ...
        State.Connected(
            share.validate().let {
                NativeBridge.connect(
                    ...
                    it.uid,
                    it.gid,
                    it.groups.toLongArray(),
                    it.timeoutSeconds * 1000,
                    it.readOnly,
                )
            }
        )
    ...
    @Synchronized
    fun <T> useNative(block: (Long) -> T): T =
        when (val s = state) {
            is State.Connected -> block(s.pointer)
            State.Closed -> throw NfsException(Errno.EBADF, "Connection is closed")
        }
    ...
    fun remove(path: RemotePath, directory: Boolean) = useNative {
        NativeBridge.remove(it, path.value, directory)
    }

    fun rename(from: RemotePath, to: RemotePath) = useNative {
        NativeBridge.rename(it, from.value, to.value)
    }
    ...
    ): OpenedFile = useNative { s ->
        // Verify identity before a destructive truncate.
        val actual = NativeBridge.stat(s, node.path.value)
        if (actual.inode != node.entry.inode || actual.device != node.entry.device)
            throw NfsException(Errno.ESTALE, "Document was replaced on the server")
        val f =
            NativeBridge.open(
                s,
                node.path.value,
        ...
        try {
            ...
            val io =
                object : FileIo {
                    ...
                    private fun <T> io(block: (Long) -> T): T =
                        try {
                            useNative(block)
                        } catch (t: NfsException) {
                            healthy = false
                            throw t
                        }

                    override fun read(offset: Long, size: Int, data: ByteArray) = io {
                        NativeBridge.read(it, f, offset, size, data)
                    }
                    ...
                    override fun write(offset: Long, size: Int, data: ByteArray) = io {
                        ...
                        try {
                            WriteReceipt(
                                NativeBridge.write(it, f, position, size, data),
                                position + size,
                            )
                        } finally {
                            written()
            ...
            OpenedFile.own(
                if (mode == OpenMode.READ && share.readPolicy == ReadPolicy.ReadAhead)
                    ReadAhead(io, revision)
                else io,
                mode,
                size,
            )
        } catch (@Suppress("TooGenericExceptionCaught") t: Throwable) {
            runCatching { NativeBridge.close(s, f) }
            throw t
        }
    }
...
internal class NfsBackend {
    ...
    fun drain(ticket: OperationGate.Drain? = requestDrain()): List<Throwable> {
        ...
        try {
            ticket.awaitIdle()
            openFiles.keys.toList().forEach { file ->
                try {
                    file.close()
    ...
    fun invalidate(share: Share) {
        pools.remove(share)?.close()
    }

    // The nested scopes enforce operation and session ownership around every mutation.
    @Suppress("NestedBlockDepth")
    fun <T> metadata(share: Share, action: (NativeSession) -> T): T =
        gate.enter().use {
            pools(share).metadata.acquire().use { lease ->
                try {
                    action(lease.value)
                } catch (t: NfsException) {
                    if (t.isTransportFailure || t.errno == Errno.ESTALE) lease.reusable = false
                    throw t
                }
            }
        }

    fun <T> readMetadata(share: Share, read: (MetadataReader) -> T): T =
        gate.enter().use {
            MetadataReads(
                    acquire = {
                        // A stop can begin during the first attempt. Do not reconnect while
                        // draining.
                        if (gate.state() != OperationGate.State.Running)
                            throw NfsException(
                                Errno.ENOTCONN,
                                "Connections are stopping. Open NFS SAF and start " +
                                    "connections again.",
                            )
                    ...
                    onRetry = { error ->
                        android.util.Log.w(
                            "NfsSaf",
                            "Metadata connection failed (errno=${error.errno}); " +
                                "discarded session, retrying once",
                        )
                    },
                )
                .run(read)
    ...
    fun open(share: Share, node: Node.File, mode: OpenMode): OpenedFile =
        gate.enter().use {
            if (share.readOnly && mode.writable)
                throw NfsException(Errno.EROFS, "Share is read-only")
            ...
            try {
                ...
                file =
                    lease.value.open(
                        node,
                        mode,
                        { revisions.get(stripe) },
                        { revisions.incrementAndGet(stripe) },
                    ) { reusable ->
